...
sk_sp<SkTypeface> OsmAnd::SkiaUtilities::createTypefaceFromData(const QByteArray& data)
{
    ...
    // auto fontDataStream = std::unique_ptr<SkStreamAsset>(new SkMemoryStream(data.constData(), data.length(), true));
    typeface = SkTypeface::MakeFromStream(std::unique_ptr<SkStreamAsset>(new SkMemoryStream(data.constData(), data.length(), true)));
